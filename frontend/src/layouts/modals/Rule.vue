    ...
    <el-form label-position="top">
      <div class="form-grid">
        ...
        <el-form-item :label="$t('objects.outbound')">
          <el-select v-model="rule.outbound" clearable filterable>
...
void props.clients; void props.inTags; void props.rsTags

const rule = ref<any>({ action: 'route', outbound: '' })
...
const ruleMode = computed({
  ...
  set: (v: string) => {
    if (v === 'logical') {
      rule.value = { type: 'logical', mode: 'and', rules: [], action: rule.value.action ?? 'route', outbound: rule.value.outbound }
    } else {
      rule.value = { action: rule.value.action ?? 'route', outbound: rule.value.outbound }
    }
  },
})
...
const updateData = () => {
  if (props.index >= 0) {
    ...
    rule.value = { action: 'route', outbound: '' }
    title.value = 'add'
  }
}
